...
public:
  ...
  {
    printf("Trusted ca authorities sz=%u\n", m_stores.size());    
					 ...
					 bool with_client_certs = false)
{
  ...
  printf("create_creds made ca_opts \n");
...
public:
  Application(Connection_ptr remote) :
    ...
    m_socket(remote)
  {
    printf("Application %p set up\n", this);
    m_socket->on_read(4096, [this](auto buf, size_t n) { this->on_client_write(buf.get(), n); });
  }

  void on_client_write(const uint8_t* buf, size_t n)
  {
    printf("Got %d bytes from client in %p\n", n, this);
    try
      {
...
std::map<std::string, std::unique_ptr<Application>> g_apps;
...
void Service::start(const std::string&)
{
  ...
  server.on_connect(
    ...
    printf("Botan TLS threw something \n");
  }
}
